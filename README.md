...
### <p align="center">Завершение</p>

<br>

1. Проверка конфигурации системы и настроек:

+ Проверить файлы конфигурации, такие как `/etc/fstab`, `/etc/network/interfaces`, `/etc/default/grub (для GRUB)` или `/boot/efi/loader/loader.conf` и `/boot/efi/loader/entries/<основное_имя_ос>.conf` (для systemd-boot), чтобы убедиться, что они содержат правильные настройки и пути.

2. Перезагрузка системы:

+ В командной строке введите следующую команду, чтобы перезагрузить систему:
...
После перезагрузки будет возможность проверить, загружается ли Linux успешно с установленного диска.

3. Проверка загрузки Linux:

+ При загрузке виртуальной машины или компьютера обратить внимание на экран загрузки, где нужно увидеть информацию о загрузчике (например, GRUB или systemd-boot).

+ Убедиться, что загрузчик отображает правильные варианты загрузки операционной системы (например, Ubuntu) и что можно выбрать нужный вариант.

+ Загрузить операционную систему, выбрав соответствующий вариант загрузки.

+ Если Linux успешно загружается и видно рабочий стол или командную строку, это означает, что установка Linux завершена и операционная система работает корректно.
